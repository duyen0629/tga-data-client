...
namespace TgaGateway2
{
    internal partial class Program
    {
            ...
            SupabaseService supabaseService)
        {
            ...
            //// (no need) 2. Process ALL Recognition Managers (fetch, display, and save to Supabase)
            //await ProcessRecognitionManagers(supabaseService);

            //// (no need) 3. Process ALL Data Managers (fetch, display, and save to Supabase)
            //await ProcessDataManagers(supabaseService);

            //// (no need) 4. Process ALL Validation Codes (fetch, display, and save to Supabase)
            //await ProcessValidationCodes(supabaseService);

            //// (no need) 4.1 Process ALL Classification Schemes (fetch, display, and save to Supabase)
            //await ProcessClassificationSchemes(supabaseService);

            //// (no need) 4.2 Process ALL Classification Purposes (fetch, display, and save to Supabase)
            //await ProcessClassificationPurposes(supabaseService);

            //// (no need) 4.3 Process ALL Lookups (fetch, display, and save to Supabase)
            //await ProcessLookups(supabaseService);

            //// (no need) 5. Process ALL Contact Roles (fetch, display, and save to Supabase)
            //await ProcessContactRoles(supabaseService);

            //// (no need) 6. Process ALL Address States (fetch, display, and save to Supabase)
            //await ProcessAddressStates(supabaseService);

            //// (no need) 7. Process Recognition Manager Assignments (via GetDetails)
            //await ProcessRecognitionManagerAssignments(supabaseService);

            //// (no need) 8. Process Data Manager Assignments (via GetDetails)
            ...
            //// (no need) 13.Process Contacts(via GetDetails)
            // await ProcessContacts(supabaseService);

            //// (no need) 14.Process Classifications(via GetDetails)
            // await ProcessClassifications(supabaseService);

            //// (no need) 15.Process Mapping Information(via GetDetails)
            // await ProcessMappings(supabaseService);

            //// (no need) 16.Process Currency Periods(via GetDetails) check this later
            ...
            //// (no need) 20. Process Deleted Training Components (via SearchDeletedByDeletedDate)
