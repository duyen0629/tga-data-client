...
{
    /// <summary>
    /// Handler for RecognitionManagerAssignment operations - fetching and saving to database
    /// </summary>
    public static class RecognitionManagerAssignmentHandler
    {
        /// <summary>
        /// Fetches recognition manager assignments via GetDetails for each training component code
        /// </summary>
        public static async Task<List<RecognitionManagerAssignmentRecord>> ProcessRecognitionManagerAssignments(
            TrainingComponentSummaryService summaryService,
            RecognitionManagerAssignmentService assignmentService,
            SupabaseService supabaseService,
            DateTime startDate,
            DateTime endDate,
            int maxResults = 0)
        {
            Console.WriteLine("=== Getting and Saving Recognition Manager Assignments ===");
            Console.WriteLine("(Fetching details per training component code)\n");

            var allAssignments = new List<RecognitionManagerAssignmentRecord>();
            ...
            try
            {
                int totalProcessed = await summaryService.SearchByModifiedDateWithCallback(
                    ...
                    {
                        var pageAssignments = new List<RecognitionManagerAssignmentRecord>();

                        foreach (var summary in pageResults)
                        {
                            var assignments = assignmentService.GetRecognitionManagerAssignments(summary.Code);
                            foreach (var assignment in assignments)
                            {
                                pageAssignments.Add(new RecognitionManagerAssignmentRecord
                                {
                                    TrainingComponentCode = summary.Code,
                                    RecognitionManagerCode = assignment.Code,
                        ...
                        {
                            await supabaseService.SaveToSupabase(pageAssignments.ToArray(), "recognition_manager_assignments");
                        }
                ...
                Console.WriteLine($"✓ Saved {allAssignments.Count} recognition manager assignments to Supabase!");
